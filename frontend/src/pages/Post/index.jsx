...
const Post = () => {
    ...
    const getPostById = async () => {
        try {
            const response = await axios.get(`http://localhost:8000/api/posts/${Number(id)}`);
    ...
    const getCommentListById = async () => {
        try {
            const response = await axios.get(`http://localhost:8000/api/comment/${Number(id)}`);
    ...
    return (
        <Grid component={"main"}>
                ...
                <Grid sx={{ maxWidth: 600, width: '100%', padding: 2, border: '2px solid #ccc', }} container rowSpacing={4}>
                    ...
                    <Grid item xs={12}>
                        {isLoading ? (
                            ...
                            <PostDetailed handleEdit={() => setOpenModal(true)} data={data} likedBy={data.likedBy} handleDislike={handleDislike} handleLike={handleLike} />
                        )}
                    </Grid>
                    <Grid item xs={12}>
            ...
            <EditPostDialog data={data} openModal={openModal} setOpenModal={setOpenModal} getPostById={getPostById} />
        </Grid>
