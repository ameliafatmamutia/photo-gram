...
module.exports = {
  createPost: async (req, res) => {
    ...
    VALUES (${db.escape(image_url)}, ${db.escape(caption)}, ${db.escape(id_user)});`;
    try {
      ...
      return res.status(201).send({ code: 201, data: insertedPost[0], message: 'Success create new post' });
    } catch (error) {
      return res.status(500).send({ code: 201, data: req.body, message: 'Error while creating post' });
    }
  },
  ...
  getPostById: async (req, res) => {
    ...
    `;
      ...
      res.status(200).json({ code: 200, data: post, message: `success get post by id_post = ${postId}` });
    } catch (error) {
  ...
  fetchDetailedPost: async (req, res) => {
    ...
    `;
      ...
      res.status(200).json({ code: 200, message: 'Success get post with data', data: response, totalCount: totalCount[0].total });
    } catch (error) {
