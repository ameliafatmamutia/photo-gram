...
module.exports = {
  ...
  createComment: async (req, res) => {
    ...
    try {
      ...
      return res
        .status(201)
        .send({ message: `Success cooment post with id = ${id_post}` });
    } catch (error) {
  ...
  deleteComment: async (req, res) => {
    try {
      const id = req.params.id;
      let deleteCommentQuery = `DELETE FROM comments WHERE id = ${db.escape(
        id
      )}`;
      await query(deleteCommentQuery);
      return res
        .status(201)
        .send({ message: `Success delete post with id = ${id}` });
    } catch (error) {
